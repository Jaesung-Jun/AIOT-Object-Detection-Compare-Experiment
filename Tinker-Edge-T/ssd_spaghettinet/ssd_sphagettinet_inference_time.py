...
parser = argparse.ArgumentParser(description="ssd sphagettinet Test Code (Edited 2022-05-06)")
...
if edge_tpu_delegate is None:
    interpreter = tflite.Interpreter(model_path=f"{DEFAULT_DIR}lite-model_ssd_spaghettinet_edgetpu_large_320_uint8_default_1.tflite")
else:
    interpreter = tflite.Interpreter(model_path=f"{DEFAULT_DIR}lite-model_ssd_spaghettinet_edgetpu_large_320_uint8_default_1.tflite", experimental_delegates=[edge_tpu_delegate])
...
f = open(f"./ssd_sphagettinet_inference_time.txt", "a")
