...
images = np.ndarray((NUM_OF_IMAGES, 300, 300, 3), dtype=np.uint8)

for i in range(START_INDEX, END_INDEX):
    image = Image.open(image_paths[i])
    if len(np.array(image).shape) == 3:
        print(str(i) + " : " + image_paths[i])
        images[i-START_INDEX] = image.resize((300, 300))
        images[i-START_INDEX] = np.array(images[i-START_INDEX])
        images[i-START_INDEX] = np.reshape(np.array(images[i-START_INDEX], dtype=np.uint8), (1, 300, 300, 3))
...
if edge_tpu_delegate is None:
    interpreter = tflite.Interpreter(model_path=f"{DEFAULT_DIR}ssd_mobilenet_v1_1_default_1.tflite")
else:
    interpreter = tflite.Interpreter(model_path=f"{DEFAULT_DIR}ssd_mobilenet_v1_1_default_1.tflite", experimental_delegates=[edge_tpu_delegate])
...
f = open(f"./ssd_mobilenet_v1_inference_time.txt", "a")
